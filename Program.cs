...
namespace RpaChallenge
{
    ...
    class Program
    {
        ...
        {
            string path = @"C:\Users\lsilvpin\source\repos\RpaChallenge\Log.txt";
            if (!File.Exists(path))
            {
        ...
        private static bool KeepTryingUntil(string actionName, string xpath, int timeOut, string inputKeys = "")
        {
            if (actionName == "Click")
            {
                ...
                {
                    Thread.Sleep(100);
                    if (_driver.FindElements(By.XPath(xpath)).Count != 0)
                    {
                        _driver.FindElement(By.XPath(xpath)).Click();
                    }
                    duration++;
                } while (_driver.FindElements(By.XPath(xpath)).Count == 0 && duration*100 < timeOut);
                return true;
            }
        ...
        private static bool MoveSheetIfNecessary()
        {
            try
            {
                ...
                if (!File.Exists(@"C:\Users\lsilvpin\source\repos\RpaChallenge\challenge.xlsx"))
                {
                    File.Move(@"C:\Users\lsilvpin\Downloads\challenge.xlsx", @"C:\Users\lsilvpin\source\repos\RpaChallenge\challenge.xlsx");
                }
                else
                {
                    Logger(@"O arquivo challenge.xlsx já existia na pasta de destino C:\Users\lsilvpin\source\repos\RpaChallenge\ portanto o que foi baixado não foi movido");
                }
        ...
        private static bool XlToJson()
        {
            try
            {
                ...
                using (ExcelEngine excelEngine = new ExcelEngine())
                {
                    ...
                    FileStream fileStream = new FileStream(@"C:\Users\lsilvpin\source\repos\RpaChallenge\challenge.xlsx", FileMode.Open);
                    ...
                    //open file stream
                    using (StreamWriter file = File.CreateText(@"C:\Users\lsilvpin\source\repos\RpaChallenge\challenge.json"))
                    {
        ...
        private static bool JsonToEntityAndCollect()
        {
            try
            {
                string json = File.ReadAllText(@"C:\Users\lsilvpin\source\repos\RpaChallenge\challenge.json"); // Abre o arquivo Json como string
